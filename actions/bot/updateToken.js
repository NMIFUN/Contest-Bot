...
module.exports = async (ctx) => {
  if (ctx.callbackQuery) {
    ...
    ctx.user.state = 'bot_updateToken'
    return ctx[ctx.message ? 'reply' : 'editMessageText'](
      ctx.i18n.t('bot.updateToken.text'),
      {
        parse_mode: 'HTML'
      }
    )
    ...
    if (!ctx.message.text)
      return ctx.replyWithHTML(ctx.i18n.t(`bot.updateToken.text`))
    ...
    if (!getBot || !getBot.data.ok)
      return ctx.replyWithHTML(ctx.i18n.t(`bot.updateToken.error`))

    ctx.user.state = null

    const [bot, count] = await Promise.all([
      ctx.Bot.findOneAndUpdate(
